...
HOST_FLAGS := ${COMMON_FLAGS} -std=c11 -O3 `dpu-pkg-config --cflags --libs dpu` -D${TYPE} -DNR_TASKLETS=${NR_TASKLETS} -DNR_DPUS=${NR_DPUS} -DEN_TYPE=${TYPEEN} -DBLNC_TSKLT_ROW=${BLNC_TSKLT_ROW} -DBLNC_TSKLT_NNZ=${BLNC_TSKLT_NNZ}  -DCG_TRANS=${CG_TRANS} -DFG_TRANS=${FG_TRANS} -fopenmp
